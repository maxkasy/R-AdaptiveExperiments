...
ExpectedRegret = function(wavesizes, C, theta, methods, R) {
  ...
  for (i in methods) {
    # pick here which methods to simulate
    ...
    regretTWave = future_sapply(1:R, function(j)
      SimulateTWaveDesign(wavesizes, C, theta, Methods[i]))
    regretTable = rbind(get0("regretTable"),
                        tibble(
                          Statistic = paste("$\\quad$ ", MethodNames[i], sep = ""),
    ...
    shareTreatments[[Methods[i]]] = table(factor(regretTWave[1,], levels =
                                                   max(theta) - theta)) #store shares assigned to each treatment for method i
  }
  
  lastrows = tibble(Statistic = c("Units per wave"),
                    #, "Number of treatments"),
                    Value = c(wavesizes[1]))#, k))
  
  tablecolumns = rbind(
    tibble(Statistic = "Average regret", Value = NA),
    regretTable,
...
DesignTable = function(DataList,
  ...
  if (!is.null(columnames)) {
    ...
    colnames(RegretTable) = c("Statistic", map(DataList, function(data)
      paste(
        "$\\theta = (", toString(data$theta, sep = ", "), ")$"
      )))
  }
  
  #write to tex file, plot histograms
  if (!is.null(filename)) {
    PrintRegretTable(RegretTable,
                     filename,
                     DataList[[1]]$dataname,
                     MC_replicates,
                     length(methods))
    if (DoHistograms)
      PrintRegretHistogram(
        shareTreatmentsList,
        filename,
        DataList[[1]]$dataname,
        #paste(DataList[[1]]$filename, " et al.", sep=""),
        MC_replicates,
        map_dbl(DataList, "waves")
      )
  }
}
...
PrintRegretTable = function(RegretTable,
                            filename,
                            caption,
                            MC_replicates,
                            nmethods) {
  ...
  digs = matrix(
    c(
      rep(4, (nmethods+1) * (cols + 1)),
      rep(3, (nmethods+1) * (cols + 1)),
      rep(0, remainderrows * (cols + 1))
    ),
    nrow = rows + remainderrows,
    ncol = cols + 1,
    byrow = T
  ) #controling digits
  print(
    xtable(
      ...
      type = "latex",
      caption = caption,
      #paste(MC_replicates, " replications, ", waves, " waves.", sep=""),
      label = paste("tab:", labtext, sep = ""),
      digits = digs
    ),
    hline.after = c(-1, 0, nmethods + 1, rows, rows + remainderrows),
    #horizontal lines
    file = paste("../Figures/Simulations/", labtext, ".tex", sep =
                   ""),
    caption.placement = "top",
    latex.environments = "center",
    #centering the table and caption
    include.rownames = FALSE,
    #to omit row numbering
    sanitize.text.function = function(x) {
      x
    }
  ) #to maintain tex strings as intended
}




PrintRegretHistogram = function(shareTreatmentsList,
                                filename,
                                dataname,
                                MC_replicates,
                                waves) {
  ...
  for (i in 1:length(columnames)) {
    ...
    tibble_i = tibble(
      regret = as.double(names(shareTreatments[[1]])),
  ...
  #print histograms
  ggplot(histTibble[histTibble$regret >= 0,], aes(x = regret, y = probability, fill =
                                                    algorithm)) +
    geom_col(width = maxregret / 15,
             position = position_dodge2(padding = .2, preserve = c("single"))) +
    scale_fill_manual(
      labels = c("non-adaptive", "modified Thompson"),
