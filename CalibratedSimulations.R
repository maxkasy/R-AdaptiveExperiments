  ...
  for (Mult in multiplicities) {
    for (application in 1:length(DataList)) {
      ...
      for (i in 1:length(wavenumbers)) {
        applicationData[[i]]$wavesizes = rep(floor(applicationData[[i]]$N * Mult / wavenumbers[i]) ,
                                             wavenumbers[i])
        applicationData[[i]]$waves = wavenumbers[i]
      }
      filename = paste(today,
                       "_CalibratedSimulations_",
                       DataList[[application]]$filename,
                       "_" ,
                       Mult,
                       sep = "")
      DesignTable(applicationData,
                  methods,
                  MC_replicates,
                  columnames,
                  filename,
...
if (DoCovariates) {
  ...
  for (Mult in multiplicities) {
    for (application in 1:length(DataList)) {
      ...
      filename = paste(
        today,
        "_Covariates_CalibratedSimulations_",
        DataList[[application]]$filename,
        "_" ,
        Mult,
        sep = ""
      )
      DesignTableCovariates(applicationData,
                            methods,
                            MC_replicates,
                            columnames,
                            filename)
